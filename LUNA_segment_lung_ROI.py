...
working_path = "D:/LUNA_CTscans/processed_data/"
file_list = glob(working_path + "images_*.npy")

def visualize_img_hist(img):
	fig = plt.figure()
...
for img_file in file_list:
	imgs_to_process = np.load(img_file)
...
for fname in tqdm(file_list):
	...
	for i in range(len(imgs_to_process)):
		...
		img = imgs_to_process[i]
		new_size = [512, 512]		# scaling back to the original size of image
		...
		img[img == old_min] = new_mean - 1.2 * new_std		# resetting background color
		img = img - new_mean
		img = img / new_std
		# make bounding box (min row, min col, max row, max col)
...
imgs = np.load(working_path + 'images_207.npy')
lungmask = np.load(working_path + 'lungmask_207.npy')
